...
export function ScienceLessonPlayer({ lesson, onBack, setActiveTab: _setActiveTab }: Props) {
  ...
  return (
    <div className="space-y-4 text-white">
      ...
      <header className="rounded-lg border border-white/10 bg-gradient-to-r from-[#062019] via-[#06121e] to-[#050711] p-5">
      ...
      {tab === 'explore' && (
        <div className="grid gap-3 lg:grid-cols-[1.05fr_1fr]">
          <div className="space-y-3">
            <Card title="Student explanation" icon={BookOpen} color={theme.primary}>
              <p className="text-sm leading-6 text-slate-200">{lesson.studentExplanation}</p>
            </Card>
            <Card title="Animated steps" icon={Sparkles} color={theme.primary}>
            ...
            <Card title="Objectives" icon={Target} color={theme.primary}>
            ...
            <Card title="Teacher notes" icon={GraduationCap} color={theme.primary}>
              ...
              <div key={q.id} className="rounded-lg border border-white/10 bg-white/[.03] p-4">
                ...
                {q.choices ? (
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="mt-2 w-full rounded-md border border-white/10 bg-[#0a1a14] p-2 text-sm text-white placeholder:text-slate-500 focus:border-white/30 focus:outline-none"
                  />
                )}

                <div className="mt-2 flex items-center gap-2">
                  <button
                    onClick={() => setPracticeRevealed((prev) => ({ ...prev, [q.id]: !prev[q.id] }))}
